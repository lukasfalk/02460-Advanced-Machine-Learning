...
class MultivariateGaussianDecoder(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        logits = self.decoder_net(z)
        return td.Independent(td.Normal(logits, scale=torch.exp(self.log_std)), 2)
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        x: [torch.Tensor] 
           A tensor of dimension `(batch_size, feature_dim1, feature_dim2)`
        """
        return -self.elbo_GoM(x) # Change to elbo(x) if using Gaussian prior
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        # Test model
        evaluate_elbo_GoM(model, mnist_test_loader, args.device)
