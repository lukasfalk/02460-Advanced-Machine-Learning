...
class SimpleGraphConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        node_state_spectral = torch.zeros_like(X)
        eigen_values, U = torch.linalg.eigh(A)
        U_inv = U.T
        for k in range(self.filter_length):
            lambda_k = torch.diag(eigen_values**k)
            node_state_spectral += self.h[k] * (U @ lambda_k @ U_inv @ X)
        # ---------------------------------------------------------------------------------------------------------
